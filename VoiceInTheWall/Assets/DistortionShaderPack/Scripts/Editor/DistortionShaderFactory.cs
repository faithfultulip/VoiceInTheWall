...
namespace nightowl.distortionshaderpack
{
    public class DistortionShaderFactory
    {
        ...
        {
            if (!config.UseLWRP) AddLine("Tags { \"Queue\" = \"Transparent\" \"RenderType\" = \"Transparent\" \"IgnoreProjector\" = \"True\" }");
            if (config.UseLWRP) AddLine("Tags { \"Queue\" = \"Transparent\" \"RenderType\" = \"Opaque\" \"RenderPipeline\" = \"LightweightPipeline\" \"IgnoreProjector\" = \"True\" }");
            if (!config.UseLWRP) AddLine("Blend SrcAlpha OneMinusSrcAlpha");
            AddLine("ZWrite Off");
        }
        ...
        private static void AddSubShader()
        {
            ...
            AddLine("{");
            tabs++;
            if (!config.UseLWRP && !config.UseRenderTexture) AddGrabPass();
            AddMainPass();
            tabs--;
            AddLine("}");
        }
        ...
        private static void AddMainPass()
        {
            ...
            tabs++;
            AddLine("Name \"BASE\"");
            if (!config.UseLWRP) AddLine("Tags { \"LightMode\" = \"Always\" }");
            if (config.UseLWRP) AddLine("Tags { \"LightMode\" = \"LightweightForward\" }");
            AddCGProgram();
            tabs--;
            AddLine("}");
        }
        ...
        {
            AddLine("");
            if (config.UseLWRP)             AddLine("sampler2D _CameraOpaqueTexture;");
            if (!config.UseLWRP)            AddLine("sampler2D _GrabTexture;");
        ...
        private static void AddFragmentCode()
        {
            ...
            string grabTexture = "_GrabTexture";
            if (config.UseLWRP) grabTexture = "_CameraOpaqueTexture";
        ...
        private static void AddVectorFromCenterCode()
        {
            ...
            if (config.UseLWRP) screenX = "(_ScreenParams.x*2)";
        ...
        private static void AddNormalCode()
        {
            ...
            if (config.UseLWRP) uvCorrection = "1";
